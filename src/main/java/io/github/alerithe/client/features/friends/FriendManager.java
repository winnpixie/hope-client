...
public class FriendManager extends FeatureManager<Friend> {
    @Override
    public void load() {
        setDataFile(new File(Client.DATA_DIR, "friends.properties"));
        ...
        Client.COMMAND_MANAGER.add(new FriendCommand());
    }
    ...
    private class FriendCommand extends Command {
        ...
        private void listFriends(int page) {
            int pageCount = (getChildren().size() - 1) / COMMANDS_PER_PAGE; // 7 FRIENDS PER PAGE
